...
export default function PyqPage() {
  const [activeTab, setActiveTab] = useState<'year' | 'subject' | 'database'>('year');
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10 sm:py-20 md:py-28 fade-up">
        ...
        <div className="text-center mb-10 sm:mb-16 relative flex flex-col items-center">
          ...
          <h1 className="heading text-4xl sm:text-5xl md:text-7xl lg:text-8xl text-[var(--foreground)] mt-2 mb-4 sm:mb-6 transition-all relative z-10">
            {activeTab === 'database' ? 'PYQ DATA BASE' : <>CHOOSE YOUR <span className="text-[var(--accent)] drop-shadow-[0_0_15px_rgba(163,230,53,0.3)]">{activeTab === 'year' ? 'YEAR' : 'SUBJECT'}</span></>}
          </h1>
          ...
          {/* Tab Toggle */}
          <div className="inline-flex flex-wrap items-center p-1 rounded-[2rem] border border-[#333] bg-[#0e0e0e] relative z-10 w-full max-w-[32rem] transition-shadow hover:shadow-[0_0_20px_rgba(255,255,255,0.05)] mx-auto">
            <button
              ...
              className={`flex-1 min-w-[100px] rounded-full py-3 text-xs md:text-sm font-bold uppercase tracking-widest transition-all ${
                activeTab === 'year' 
                  ? 'bg-[var(--accent)] text-black shadow-[0_0_15px_rgba(163,230,53,0.3)]' 
          ...
          {activeTab === 'year' && (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
              {YEARS.map((year) => (
                <div
                  key={year}
                  className="group relative flex flex-col rounded-[1rem] bg-[var(--background-secondary)] p-4 sm:p-6 border-2 border-[#262626] transition-all hover:bg-[var(--background)] hover:border-[#333] hover:shadow-[0_0_30px_rgba(0,0,0,0.5)] animate-in fade-in zoom-in duration-300"
                >
                  {/* Year Heading */}
                  ...
                  <div className="flex flex-col gap-3 w-full mt-auto relative z-10">
                     ...
                     <div className="grid grid-cols-2 gap-3 w-full">
                       <Link 
                         ...
                         className="flex items-center justify-center text-center w-full h-12 px-2 rounded-lg bg-[#1a1a1a] text-[var(--muted)] border border-[#333] text-[10px] font-bold uppercase tracking-wider leading-tight hover:text-white hover:border-[#555] hover:bg-[#222] transition-all"
                       >
                         Sectional Test
                       </Link>
                       <Link 
