...
async function main() {
    ...
    Your task is to extract the correct answer option (A, B, C, or D) AND the exact first 60 characters of the question text for all 100 questions.
    ...
    - "snippet": The first 60 characters of the question prompt text (excluding the "1 S&T" category headers etc. Just the actual question text like "Consider the following statements:")
    - "answer": The uppercase string "A", "B", "C", or "D".
    ...
    try {
        ...
        fs.writeFileSync('data/visionias-parsed-key-with-text.json', finalJson);
        console.log("Successfully parsed and saved to data/visionias-parsed-key-with-text.json");
        
        const parsed = JSON.parse(finalJson);
        console.log(`Found ${parsed.length} answers.`);
    } catch (e) {
